...
| 회원 인가 | 인증 회원 기준 주문 생성·취소, 결제 준비, 장바구니 변경, 쿠폰 발급·검증, 마이페이지·리뷰 조회 |
...
| `PaymentApplicationService` | PG 요청·승인·취소, 결제 시도 식별과 내부 처리 순서 조정, 실패 보상 호출 |
| `PaymentCompleteService` | 트랜잭션 안에서 주문·결제 Aggregate를 다시 조회하고 내부 데이터를 일괄 반영 |
| `PaymentAttempt` | Ready 단계의 일회성 `attemptKey`, `tid`, 주문·회원·예상 금액 보관을 위한 결제 시도 모델 |
...
JWT 필터가 토큰의 회원 ID로 `CustomUserDetails`를 구성합니다. 주요 개인 리소스 API는 요청에서 `memberId`를 받지 않고 `@AuthenticationPrincipal`의 `getUserId()`를 사용합니다.
...
| 장바구니 등록·조회 | 인증 회원 ID로 등록하고 조회 |
| 장바구니 수정·삭제 | `cartId + memberId`로 소유권을 확인한 뒤 변경 |
| 쿠폰 발급·적용 검증 | URL의 `memberId`를 제거하고 인증 회원 ID 사용 |
| 내 정보·내 리뷰 | 인증 회원 ID 사용 |
| 주문 내역 | 클라이언트 `memberId` 없이 인증 회원 ID를 Repository 조회 조건에 적용 |

카카오페이 승인 리다이렉트는 Bearer JWT를 첨부할 수 없는 브라우저 GET 요청이므로 일반 회원 API와 다른 검증 방식이 필요합니다. 현재 구현 상태는 [카카오페이 Ready와 승인 리다이렉트](#카카오페이-ready와-승인-리다이렉트)에 정리했습니다.
...
sequenceDiagram
    ...
    Browser->>App: 결제 준비 요청
    App->>DB: 주문 소유권 확인
    App->>PG: Ready API
    PG-->>App: tid와 결제 URL
    App->>DB: PaymentAttempt 저장
    App-->>Browser: 결제 URL
    PG-->>Browser: approval_url로 리다이렉트
    Browser->>App: GET 승인 URL(attemptKey, pg_token)
    App->>DB: 결제 시도·주문·예상 금액 조회
    App->>PG: Approve API
    PG-->>App: 승인 결과
...
| 단계 | 트랜잭션 | 처리 내용 |
| --- | --- | --- |
| PG Ready·Approve·Cancel | 장기 DB 트랜잭션 밖 | 외부 HTTP 요청 |
| 결제 시도 저장 | `PaymentRepository.insertPaymentAttempt()` | Ready 응답 후 `tid`와 주문·회원·예상 금액 저장 시도 |
...
### 카카오페이 Ready와 승인 리다이렉트

결제 준비 시 UUID 기반 `attemptKey`를 생성하고, Ready 응답의 `tid`를 주문 ID, 회원 ID, 예상 금액과 함께 `PaymentAttempt`에 저장하려는 구조가 추가됐습니다. 승인 진입점은 카카오페이가 브라우저를 이동시키는 `GET /payment/kakao/approve/{attemptKey}?pg_token=...` 형태입니다.

```text
attemptKey → PaymentAttempt → tid, orderId, memberId, expectedAmount
```

승인 처리에서는 `attemptKey`로 결제 시도를 조회하고 저장된 `tid`와 카카오가 전달한 `pg_token`으로 Approve API를 호출합니다. Bearer JWT를 실을 수 없는 외부 리다이렉트이므로 이 경로는 별도로 공개하고, 충분히 무작위적인 키·만료 시간·단일 사용 상태 전이로 검증해야 합니다.

현재 코드는 이 모델을 도입하는 중이지만 URL, 보안 설정과 엔티티 매핑이 아직 연결되지 않아 그대로는 Ready부터 승인까지 완료되지 않습니다. 구체적인 오류는 [현재 상태와 보완 과제](#현재-상태와-보완-과제)에 명시했습니다.
...
동시 주문으로 재고가 음수가 되는 것을 방지하기 위한 조건입니다. UPDATE 영향 행 수도 검사하며 조건에 맞는 행이 없어 결과가 `0`이면 재고 부족 예외를 발생시켜 결제 완료 트랜잭션을 롤백합니다.
...
- `PaymentAttempt`: 외부 리다이렉트의 `attemptKey`와 Ready 응답의 `tid`를 서버 데이터에 연결합니다.
- 조건부 UPDATE: 상품 ID와 가용 재고 조건을 묶어 원자적으로 재고를 차감합니다.
- 애플리케이션 조정 서비스: 주문·결제 유스케이스를 조정해 서비스 간 순환 참조를 제거합니다.
- 리소스 소유 조건: 직접 입력받은 회원 ID 대신 인증 회원 ID를 사용하고 주문·장바구니·마이페이지 조회 조건에 소유자를 포함합니다.
...
| `POST` | `/cart` | 인증 회원의 장바구니에 상품과 선택 옵션 추가 |
| `GET` | `/cart` | 인증 회원의 장바구니 조회 |
| `PUT` | `/cart` | 인증 회원 소유 장바구니의 수량 변경 |
...
| `GET` | `/payment/kakao/approve/{attemptKey}?pg_token={pgToken}` | 카카오페이 승인 리다이렉트 수신 및 내부 결제 완료 처리 |
...
| `GET` | `/my-page/order` | 인증 회원의 주문 내역 검색 및 페이지 조회 |
...
- 카카오페이 키·URL이 코드에 고정되어 있습니다. 운영 비밀값은 환경 변수 또는 Secret Manager로 분리해야 합니다.
- CORS 정책, OpenAPI, DB 마이그레이션, 자동화 테스트와 CI가 아직 없습니다.

## 개선 우선순위

1. Ready 단계의 결제 시도를 저장하고 승인 리다이렉트 검증, 상태 전이와 멱등성을 구현합니다.
2. PG 취소 실패를 영속화하고 재시도 및 정산 배치로 PG와 내부 주문 상태의 불일치를 복구합니다.
3. 결제·포인트 이력 모델을 보완합니다.
4. 주문 생성, 0원 결제, 승인 성공, DB 실패 후 PG 취소, 취소 실패 재시도, 중복 승인, 동시 재고 차감 통합 테스트를 작성합니다.
5. PG 구현을 Spring Bean 기반 Gateway로 전환하고 설정과 HTTP Client를 주입합니다.
6. Flyway, OpenAPI, 표준 오류 응답, CI 및 보안 설정을 정리합니다.
