...
- `POST /cart`는 여전히 요청 Body의 `memberId`를 사용하고, `PUT /cart`는 `cartId`만으로 조회·수정합니다. 두 경로 모두 인증 회원 ID를 사용한 소유권 검증이 필요합니다.
- `GET /my-page/order`는 요청의 `memberId`와 `memberSeq`를 검색 조건으로 사용하며 인증 회원을 필수 조건으로 강제하지 않습니다. 회원 조건이 누락되면 다른 주문이 조회될 수 있습니다.
- `/payment/request`는 주문 소유권을 확인하지만 회원명·상품명·결제금액·세금 값을 요청 Body에서 그대로 PG에 전달합니다. 주문 조회 결과로 PG 요청을 서버에서 다시 구성해야 합니다.
- 승인 금액 검증이 `compareTo(...) < 0`으로 구현돼 있어 실제로는 주문 금액보다 많이 결제된 경우만 취소합니다. 서버 주문 금액과 승인 금액이 정확히 같은지 `compareTo(...) != 0`으로 검사해야 합니다.
...
- `ProductRepository.updateMinusQuantity()`는 조건부 UPDATE의 영향 행 수를 확인하지 않아 동시 주문으로 재고가 먼저 소진돼도 성공으로 처리할 수 있습니다.
- `CartService.update()`에는 트랜잭션이 없고 Repository가 `executeUpdate()`를 호출하므로 `TransactionRequiredException`이 발생할 수 있습니다.
- `PaymentElement`의 쿠폰·포인트 및 취소 상세 저장 시 일부 `nullable = false` 필드가 채워지지 않습니다. 결제수단별 필수 컬럼과 승인·취소 이력 관계를 정리해야 합니다.
- 포인트 취소 분기의 문자열 비교가 `==`를 사용하며 기존 포인트 이력을 삭제합니다. 값 비교는 `equals()`로 바꾸고 취소는 역거래 이력으로 남기는 편이 적합합니다.
- `CustomUserDetails.getUsername()`은 회원 ID가 아니라 회원명을 반환합니다. Spring Security의 사용자 식별 계약과 일관되게 회원 ID를 반환하도록 정리하는 편이 안전합니다.
