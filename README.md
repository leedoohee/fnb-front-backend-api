...
단순 CRUD보다 **주문 조건 검증과 금액 계산**, **PG 승인과 내부 데이터 반영의 분리**, **실패 시 승인 취소**를 주요 설계 과제로 삼았습니다. 주문과 결제의 진입 흐름은 `CheckoutService`가 조정하고, 외부 PG 통신과 결제 후 내부 반영을 별도 서비스로 구분했습니다.
...
- `PaymentCompleteService`의 승인·취소 메서드에 단일 트랜잭션 경계가 없어 재고·쿠폰·포인트·결제·주문 상태가 각각 부분 반영될 수 있습니다.
- 일부 Repository 쓰기 메서드는 트랜잭션 없이 `executeUpdate()`를 호출하므로 결제 완료·취소 경로에서 `TransactionRequiredException`이 발생할 수 있습니다.
- 조건부 재고 차감에서 `CriteriaUpdate.where()`를 두 번 호출해 앞의 상품 ID 조건이 대체될 수 있습니다. 상품 ID와 재고 조건을 하나의 `and` 조건으로 묶고 영향 행 수를 검사해야 합니다.
- `PaymentApproveCommand`와 취소 Command가 `Order` JPA 엔티티를 서비스 경계 밖에서 전달합니다. Command에는 주문 ID와 승인 값만 전달하고 내부 트랜잭션에서 다시 조회하는 방식이 안전합니다.
...
- 쿠폰 적용 검증에서 요청 값이 없을 때 null 처리 순서와 상품의 쿠폰 허용 조건을 추가 검증해야 합니다.
- 포인트 취소 분기에서 문자열을 `==`로 비교하므로 `equals()` 기반 비교로 변경해야 합니다. 사용 포인트 반환과 적립 포인트 회수 이력도 삭제보다 역거래로 남기는 편이 적합합니다.
...
1. `PaymentCompleteService`를 결제 내부 반영의 단일 `@Transactional` 경계로 만들고 하위 Repository가 같은 트랜잭션에 참여하도록 정리합니다.
2. 조건부 재고 차감 쿼리와 영향 행 수 검증을 수정합니다.
3. 승인 금액·주문 상태 검증과 결제 멱등성을 추가합니다.
4. 취소 실패 작업을 영속화하고 재시도·정산 배치로 승인/주문 불일치를 복구합니다.
5. 주문 생성, 0원 결제, 승인 성공, DB 실패 후 PG 취소, 중복 승인, 동시 재고 차감 통합 테스트를 작성합니다.
6. PG 구현을 Spring Bean 기반 Gateway로 전환하고 설정과 HTTP Client를 주입합니다.
7. Flyway, OpenAPI, CI 및 보안 설정을 정리합니다.
